...
export function DashboardClient() {
  ...
  // PRIORITY CHECK: Redirect Agency owners FIRST (before any UI renders)
  useEffect(() => {
    if (!email) return

    const checkAgencyRedirect = async () => {
      try {
        // Quick tier check
        const response = await fetch(`/api/user-access?email=${encodeURIComponent(email)}`)
        const data = await response.json()
        const tier = data.tier_name || 'Starter'

        // If not Agency tier, skip
        if (tier !== 'Agency') return

        // Agency tier detected - check if they're an organization owner
        const {
          data: { session },
        ...
        if (session?.access_token) {
          ...
          // If they own an organization, redirect immediately
          if (orgData.organization && orgData.role === 'owner') {
            console.log('Redirecting Agency owner to /dashboard/agency')
            window.location.href = '/dashboard/agency'
            // DO NOT set any loading states to false - let redirect happen
          }
        }
      } catch (error) {
        console.error('Error checking Agency redirect:', error)
        // Continue to normal dashboard on error
      }
    }
  ...
  useEffect(() => {
    ...
    const checkAccess = async () => {
      ...
      try {
        ...
        // NOTE: Agency redirect now handled in separate useEffect above
      } catch (error) {
