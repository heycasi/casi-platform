...
function SignUpForm() {
      ...
      if (data.user) {
        try {
          ...
          if (linkData.linked) {
            ...
            // Create Starter subscription for new user (or Pro Trial if code present)
            try {
              await fetch('/api/create-starter-subscription', {
                ...
                body: JSON.stringify({
                  email: email,
                  userId: data.user.id,
                  trialCode: trialCode, // Pass trial code
                }),
              })
              setMessage('Account created! Please check your email to verify your account.')
            } catch (starterError) {
              console.error('Error creating subscription:', starterError)
              setMessage('Account created! Please check your email to verify your account.')
            }
          }
        } catch (linkError) {
  ...
  return (
    ...
    >
      ...
      >
        {trialCode === 'vip7' ? (
          <span style={{ color: '#B8EE8A', fontWeight: '600' }}>
            ✨ 7-Day Free VIP Trial Applied!
          </span>
        ) : (
